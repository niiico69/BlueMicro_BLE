 ...
 */
// Row 1
#define BP_NDSH RGUI(BP_DLR)  // –
#define BP_MDSH RGUI(BP_DQUO) // —
#define BP_LABK RGUI(BP_LDAQ) // <
#define BP_RABK RGUI(BP_RDAQ) // >
#define BP_LBRC RGUI(BP_LPRN) // [
#define BP_RBRC RGUI(BP_RPRN) // ]
#define BP_CIRC RGUI(BP_AT)   // ^
#define BP_PLMN RGUI(BP_PLUS) // ±
#define BP_MMNS RGUI(BP_MINS) // −
#define BP_DIV  RGUI(BP_SLSH) // ÷
#define BP_MUL  RGUI(BP_ASTR) // ×
#define BP_NEQL RGUI(BP_EQL)  // ≠
#define BP_PERM RGUI(BP_PERC) // ‰
// Row 2
#define BP_PIPE RGUI(BP_B)    // |
#define BP_ACUT RGUI(BP_EACU) // ´ (dead)
#define BP_AMPR RGUI(BP_P)    // &
#define BP_OE   RGUI(BP_O)    // Œ
#define BP_DGRV RGUI(BP_EGRV) // ` (dead)
#define BP_IEXL RGUI(BP_DCIR) // ¡
#define BP_CARN RGUI(BP_V)    // ˇ (dead)
#define BP_ETH  RGUI(BP_D)    // Ð
#define BP_DSLS RGUI(BP_L)    // / (dead)
#define BP_IJ   RGUI(BP_J)    // Ĳ
#define BP_SCHW RGUI(BP_Z)    // Ə
#define BP_BREV RGUI(BP_W)    // ˘ (dead)
// Row 3
#define BP_AE   RGUI(BP_A)    // Æ
#define BP_UGRV RGUI(BP_U)    // Ù
#define BP_DIAE RGUI(BP_I)    // ¨ (dead)
#define BP_EURO RGUI(BP_E)    // €
#define BP_COPY RGUI(BP_C)    // ©
#define BP_THRN RGUI(BP_T)    // Þ
#define BP_SS   RGUI(BP_S)    // ẞ
#define BP_REGD RGUI(BP_R)    // ®
#define BP_DTIL RGUI(BP_N)    // ~ (dead)
#define BP_MACR RGUI(BP_M)    // ¯ (dead)
#define BP_CEDL RGUI(BP_CCED) // ¸ (dead)
// Row 4
#define BP_BSLS RGUI(BP_AGRV) // (backslash)
#define BP_LCBR RGUI(BP_Y)    // {
#define BP_RCBR RGUI(BP_X)    // }
#define BP_ELLP RGUI(BP_DOT)  // …
#define BP_TILD RGUI(BP_K)    // ~
#define BP_IQUE RGUI(BP_QEST) // ¿
#define BP_RNGA RGUI(BP_Q)    // ° (dead)
#define BP_DGRK RGUI(BP_G)    // µ (dead Greek key)
#define BP_DAGG RGUI(BP_H)    // †
#define BP_OGON RGUI(BP_F)    // ˛ (dead)
// Row 5
#define BP_UNDS RGUI(KC_SPC)  // _
 ...
 */
// Row 1
#define BP_PARA S(RGUI(BP_DLR))  // ¶
#define BP_DLQU S(RGUI(BP_DQUO)) // „
#define BP_LDQU S(RGUI(BP_LDAQ)) // “
#define BP_RDQU S(RGUI(BP_RDAQ)) // ”
#define BP_LEQL S(RGUI(BP_LPRN)) // ≤
#define BP_GEQL S(RGUI(BP_RPRN)) // ≥
#define BP_NOT  S(RGUI(BP_PLUS)) // ¬
#define BP_QRTR S(RGUI(BP_MINS)) // ¼
#define BP_HALF S(RGUI(BP_SLSH)) // ½
#define BP_TQTR S(RGUI(BP_ASTR)) // ¾
#define BP_PRIM S(RGUI(BP_EQL))  // ′
#define BP_DPRM S(RGUI(BP_PERC)) // ″
// Row 2
#define BP_BRKP S(RGUI(BP_B))    // ¦
#define BP_DACU S(RGUI(BP_EACU)) // ˝ (dead)
#define BP_SECT S(RGUI(BP_P))    // §
// Row 3
#define BP_DOTA S(RGUI(BP_I))    // ˙ (dead)
#define BP_CURR S(RGUI(BP_E))    // ¤ (dead)
#define BP_HORN S(RGUI(BP_COMM)) // ̛  (dead)
#define BP_LNGS S(RGUI(BP_C))    // ſ
#define BP_TM   S(RGUI(BP_R))    // ™
#define BP_MORD S(RGUI(BP_M))    // º
#define BP_DCMM S(RGUI(BP_CCED)) // , (dead)
// Row 4
#define BP_LSQU S(RGUI(BP_Y))    // ‘
#define BP_RSQU S(RGUI(BP_X))    // ’
#define BP_MDDT S(RGUI(BP_DOT))  // ·
#define BP_KEYB S(RGUI(BP_K))    // ⌨
#define BP_HOKA S(RGUI(BP_QUOT)) // ̉  (dead)
#define BP_DOTB S(RGUI(BP_Q))    // ̣  (dead)
#define BP_DDAG S(RGUI(BP_H))    // ‡
#define BP_FORD S(RGUI(BP_F))    // ª
// Row 5
#define BP_NNBS S(RGUI(BP_)) // (narrow non-breaking space)
